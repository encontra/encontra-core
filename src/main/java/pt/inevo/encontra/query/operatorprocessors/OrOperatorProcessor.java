...
public class OrOperatorProcessor<T> extends QueryOperatorProcessor<T> {
    ...
    public ResultSet<T> process(QueryParserNode node) {
        ...
        results = ((QueryProcessorDefaultImpl)queryProcessor).getCombiner().join(resultsParts, node.distinct, node.limit, node.criteria);

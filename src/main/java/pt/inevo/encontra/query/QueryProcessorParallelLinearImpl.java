...
public class QueryProcessorParallelLinearImpl<E extends IEntity> extends QueryProcessor<E> {
    ...
    class BooleanSearcherActor extends UntypedActor {
        ...
        protected HashMap<ActorRef, ResultSet> partialResults;
        ...
        public BooleanSearcherActor(HashMap<String, ActorRef> actors) {
            ...
            results = new ResultSetDefaultImpl();
            partialResults = new HashMap<ActorRef, ResultSet>();
        ...
        private void tryGetResult(String type) {
            ...
            ResultSet set = new ResultSetDefaultImpl();
                ...
                if (f.isCompleted()) {
                    ...
                    if (resultOption.isDefined()) {
                        ...
                        if (res.type.equals("RESULT")) {
                            ...
                            ResultSet<Result> newResults = null;
                            if (res.value instanceof Result) {
                                Result newResult = (Result) res.value;
                                partialResults.get(actor).add(newResult);
                                newResults = new ResultSetDefaultImpl();
                                newResults.add(newResult);
                            } else {
                                newResults = (ResultSet)res.value;
                            }

                            for(Result newResult: newResults){

                                //so, we don't have yet this result in the list
                                if (!results.containsResultObject(newResult.getResultObject())) {
                                    for (ResultSet part : partialResults.values()) {
                                        if (!part.containsResultObject(newResult.getResultObject())) {
                                            allHave = false;
                                            break;
                                        }
                                    }
            ...
            if (emptyResults.size() == booleanActors.size()) {
                Message empty = new Message();
                empty.type = "EMPTY";
                if (originalActor != null) {
                    originalActor.sendOneWay(empty, originalActor);
                } else {
                    future.completeWithResult(empty);
                }
            } else {
            ...
            if (m.type.equals("QUERY")) {
                ...
                for (QueryParserNode n : nodes) {
                    ...
                    if (n.predicateType.equals(Similar.class)
                        ...
                        partialResults.put(actorRef, new ResultSetDefaultImpl());
                        ...
                        partialResults.put(andActorRef, new ResultSetDefaultImpl());
            ...
            if (m.type.equals("QUERY")) {
                ...
                if (requestAnswer.isCompleted()) {
                    ...
                    if (resultOption.isDefined()) {
                        ...
                        for (; !answerMessage.type.equals("EMPTY") && results.getSize() < 20;) {

                            if (requestAnswer.isCompleted()) {
                                resultOption = requestAnswer.result();
                                if (resultOption.isDefined()) {
                                    answerMessage = (Message) resultOption.get();
                                    if (answerMessage.value instanceof ResultSet) {
                                        ResultSet<Result> resultList = (ResultSet<Result>) answerMessage.value;
                        ...
                        if (answerMessage.type.equals("EMPTY") || results.getSize() == 20) {
                            if (future != null) {
                                future.completeWithResult(results.getCopy());
